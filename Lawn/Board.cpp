...
void Board::PickZombieWaves()
{
	...
	if (mApp->IsAdventureMode())
	{
		...
		{
			mNumWaves = gZombieWaves[ClampInt(abs(mLevel) - 1, 0, 59)];
...
void Board::PickBackground()
{
	...
	case GameMode::GAMEMODE_ADVENTURE:
		...
		else if (abs(mLevel) <= 5 * LEVELS_PER_AREA)
		{
	...
	case GameMode::GAMEMODE_CHALLENGE_WORLD_7_4:
	case GameMode::GAMEMODE_CHALLENGE_WORLD_7_5:
	case GameMode::GAMEMODE_CHALLENGE_WORLD_7_6:
	case GameMode::GAMEMODE_CHALLENGE_WORLD_7_7:
...
void Board::InitLevel()
{
	...
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 10);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_IMP);
		mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
		mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_ZOMBIE_POLEVAULTER);
		mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_ZOMBIE_PAIL);
		...
		mSeedBank->mSeedPackets[9].SetPacketType(SeedType::SEED_ZOMBIE_GARGANTUAR);
	}
	else if (mApp->IsScaryPotterLevel())
	{
...
void Board::FadeOutLevel()
{
	...
	{
		mApp->mMusic->StopAllMusic();
		if (mApp->IsAdventureMode() && mLevel == 60 && !mApp->HasFinishedAdventure())
		{
...
bool Board::CanZombieSpawnOnLevel(ZombieType theZombieType, int theLevel)
{
	...
	return gZombieAllowedLevels[theZombieType].mAllowedOnLevel[ClampInt(theLevel - 1, 0, 59)];
}
...
ZombieType Board::PickZombieType(int theZombiePoints, int theWaveIndex, ZombiePicker* theZombiePicker)
{
	...
	for (int aZombieType = ZombieType::ZOMBIE_NORMAL; aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType++)
	{
		...
		if ((aZombieType == ZombieType::ZOMBIE_BUNGEE || aZombieType == ZombieType::ZOMBIE_BUNGEE_PAIL) &&
			(mApp->IsSurvivalEndless(aGameMode) || mApp->IsLastStand()))
		{
...
bool Board::RowCanHaveZombieType(int theRow, ZombieType theZombieType, int theWave)
{
	...
	if (theZombieType != ZombieType::ZOMBIE_DANCER && theZombieType != ZombieType::ZOMBIE_DANCER_COOL ||
		StageHasPool())
	{
		return true;
	}
		...
		for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++)
		{
			...
			if (aZombie && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL)
			{
				...
				if (isAllowedToAnimate && 
					(aZombie->mZombieType != ZombieType::ZOMBIE_BUNGEE && aZombie->mZombieType != ZombieType::ZOMBIE_BUNGEE_PAIL) && aZombie->mZombieType != ZombieType::ZOMBIE_DIGGER)
				{
	...
	if (mDebugTextMode == DebugTextMode::DEBUG_TEXT_COLLISION)
	{
		...
		while (IterateZombies(aZombie))
		{
			if (!aZombie->IsDeadOrDying())
			{
				...
				if ((aZombie->mZombieType == ZombieType::ZOMBIE_JACK_IN_THE_BOX || aZombie->mZombieType == ZombieType::ZOMBIE_JACK_IN_THE_BOX_CRAZY) &&
...
void Board::KeyChar(SexyChar theChar)
{
		...
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_3 || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WAR_AND_PEAS_4)
	{
		...
		if (theChar == _S('r'))
		{
			AddZombie(ZombieType::ZOMBIE_REPEATER_HEAD, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('p'))
		{
			AddZombie(ZombieType::ZOMBIE_POTATOMINE_HEAD, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('P'))
		{
			AddZombie(ZombieType::ZOMBIE_PUMPKIN_HEAD, Zombie::ZOMBIE_WAVE_DEBUG);
			return;
		}
		if (theChar == _S('c'))
		{
...
// fix I Zombie with bungee as an option
bool Board::BungeePailIsTargetingCell(int theGridX, int theGridY)
{
	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
		if (!aZombie->IsDeadOrDying() && aZombie->mZombieType == ZombieType::ZOMBIE_BUNGEE_PAIL && aZombie->mRow == theGridY && aZombie->mTargetCol == theGridX)
		{
			return true;
		}
	}
