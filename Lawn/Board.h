...
public:
	...
	int								mSunflowersPlanted;										//+GOTY @Inliothixi: 0x5794 // This is unused
	...
	int								mNukeCounter;
	int								mMarigoldCount;
	int								mGoldMagnetCount;
	bool							mSpawnedDiggerZombie;
	bool							mDiggerHasReachLeftSide;
	bool							mHadPlantedNuts;
	int								mHeldCounter;
	bool							mHeld;
	int								mHeldStartX;
	int								mHeldStartY;
...
public:
	...
	int								PickRowForNewZombie(ZombieType theZombieType, int theWave);
	...
	bool							RowCanHaveZombieType(int theRow, ZombieType theZombieType, int theWave);
	...
	bool							BungeePailIsTargetingCell(int theGridX, int theGridY);
...
bool								GetOvalRectOverlap(int theOvalX, int theOvalY, int radiusX, int radiusY, const Rect& theRect);
