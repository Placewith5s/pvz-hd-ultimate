...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
    ...
    if (IsSun())
    {
        ...
        mReanimationID = mApp->ReanimationGetID(aSunReanim);
    ...
    else if (mType == CoinType::COIN_SILVER)
    {
        ...
        mReanimationID = mApp->ReanimationGetID(aCoinReanim);
    ...
    else if (IsMoney())
    {
        ...
        if (mBoard)
        {
                ...
                && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameScene == SCENE_PLAYING) 
            {
                ...
                {
                    mApp->mPlayerInfo->mEarnedFromZenGarden += aCoinValue;
...
void Coin::Draw(Graphics* g)
{
    ...
    if (mType == CoinType::COIN_DIAMOND)
    {
        ...
        Image* aImage = IMAGE_AWARDPICKUPGLOW;
        ...
        g->DrawImage(aImage, mPosX - 56.0f, mPosY - 66.0f);
    ...
    {
        g->SetColorizeImages(true);
        Image* aImage = IMAGE_REANIM_COINGLOW;
        if (mFilterEffect != FilterEffect::FILTER_EFFECT_NONE)
            aImage = FilterEffectGetImage(aImage, FilterEffect::FILTER_EFFECT_LESS_WASHED_OUT);
        TodDrawImageCenterScaledF(g, aImage, mPosX - 14.0f, mPosY - 12.0f, mScale, mScale);
        g->SetColorizeImages(false);
    }
...
void Coin::Collect()
{
    ...
    if (IsLevelAward())
    {
        ...
        else if (mApp->IsAdventureMode() && mBoard->mLevel <= 58)
        {
            if (mType == CoinType::COIN_AWARD_MONEY_BAG)
            {
                mApp->PlayFoley(FoleyType::FOLEY_COIN);
        ...
        else if (mApp->IsAdventureMode() && mBoard->mLevel == 60 && !mApp->HasFinishedAdventure())
        {
...
bool Coin::IsLevelAward()
{
    return 
        ...
        mType == CoinType::COIN_AWARD_PRESENT || 
        mType == CoinType::COIN_AWARD_CHOCOLATE; // COIN_CHOCOLATE stupid bug
}
