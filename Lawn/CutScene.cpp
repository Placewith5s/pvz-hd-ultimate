...
void CutScene::PlaceAZombie(ZombieType theZombieType, int theGridX, int theGridY)
{
	...
	if (theZombieType == ZombieType::ZOMBIE_BUNGEE || theZombieType == ZombieType::ZOMBIE_BUNGEE_PAIL)
	{
		aZombie->mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, 0, 0);

		aZombie->mRow = 0;
		aZombie->mPosX = theGridX * 50.0f + 950.0f - WIDESCREEN_OFFSETX / 2.0f;
		aZombie->mPosY = 50.0f;
	}
	...
	else if (theZombieType == ZombieType::ZOMBIE_DOG_WALKER)
	{
		...
		if (aDog) {
			aDog->mPosX = aZombie->mPosX - 30;
			aDog->mPosY = aZombie->mPosY;
			aDog->mRenderOrder = aZombie->mRenderOrder - 1;
...
{
	if (theZombieType == ZOMBIE_BUNGEE || theZombieType == ZOMBIE_BUNGEE_PAIL)
	{
		thePosX = 0;
		thePosY = 0;
		return;
...
void CutScene::PreloadResources()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM)
	{
		...
		ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_TREEOFWISDOM_TREEFOOD, true);
		ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_TREEOFWISDOM_CLOUDS, true);
	}
		...
		for (int aZombieIndex = 0; aZombieIndex < MAX_ZOMBIES_IN_WAVE; aZombieIndex++)
		{
			...
			if (aZombieType == ZombieType::ZOMBIE_BOBSLED &&
				(mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_BOBSLED_BONANZA && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_VEHICLE_PARTY &&
					mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZOMBIE_PARTY))
			{
				continue;
			}
...
void CutScene::PlaceLawnItems()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_HEAT_WAVE)
	{
		...
		{
			mBoard->NewPlant(0, 0, SeedType::SEED_PEASHOOTER, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 1, SeedType::SEED_PEASHOOTER) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 1, SeedType::SEED_PEASHOOTER, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 2, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 2, SeedType::SEED_LILYPAD, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 2, SeedType::SEED_WALLNUT) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 2, SeedType::SEED_WALLNUT, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 3, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 3, SeedType::SEED_LILYPAD, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 3, SeedType::SEED_WALLNUT) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 3, SeedType::SEED_WALLNUT, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 4, SeedType::SEED_PEASHOOTER) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 4, SeedType::SEED_PEASHOOTER, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(0, 5, SeedType::SEED_PEASHOOTER) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(0, 5, SeedType::SEED_PEASHOOTER, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(1, 2, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK)
		{
			mBoard->NewPlant(1, 2, SeedType::SEED_LILYPAD, SeedType::SEED_NONE);
		}
		if (mBoard->CanPlantAt(1, 3, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK)
		{
...
void CutScene::StartLevelIntro()
{
	...
	else if (mApp->IsFinalBossLevel() && mApp->IsAdventureMode() && !isRestart && mApp->IsFirstTimeAdventureMode())
	{
	...
	if (mCrazyDaveTime <= 0 && mApp->mGameMode != GameMode::GAMEMODE_INTRO)
	{
		...
		{
			if (mBoard->mBackground == BackgroundType::BACKGROUND_1_DAY || mBoard->mBackground == BackgroundType::BACKGROUND_2_NIGHT || mBoard->mBackground == BackgroundType::BACKGROUND_6)
			{
...
void CutScene::CancelIntro()
{
	...
	if (mApp->mGameMode == GAMEMODE_INTRO)
	{
...
void CutScene::AddFlowerPots()
{
	...
	if (mBoard->mLevel == 41 || mBoard->mLevel == 50 || mBoard->mLevel == 51)
	{
		aPotColumns = 5;
	}
	else if (mBoard->mLevel == 42 || mBoard->mLevel == 52)
	{
		aPotColumns = 4;
	}
	else if (mBoard->mLevel >= 43 && mBoard->mLevel <= 50 && mBoard->mLevel >= 53 && mBoard->mLevel <= 59)
	{
		aPotColumns = 3;
	}
...
void CutScene::ShowShovel()
{
	...
	if (!mApp->IsFirstTimeAdventureMode() || mApp->mPlayerLevelRef > 4 || mBoard->mLevel > 4)
	{
		mBoard->mShowShovel = true;
	}
}
...
void CutScene::StartSeedChooser()
{
	...
	mApp->mSeedChooserScreen->mRandomButton->SetDisabled(false);
}
...
void CutScene::UpdateUpsell()
{
	...
	switch (mCrazyDaveLastTalkIndex)
	{
		case 3305:  // “像这个！”
		{
			...
			AttachEffect* anAttachEffect = AttachReanim(aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand")->mAttachmentID, aReanimSquash, 0, 0);
			TodScaleRotateTransformMatrix(anAttachEffect->mOffset, 92, 387, 0, 1.2f, 1.2f);
		...
		case 3306:  // “还有这个！”
		{
			...
			TodScaleRotateTransformMatrix(anAttachEffect->mOffset, -50 - 25, 230 + 25, 0.5f, 1.2f, 1.2f);
			aCrazyDaveReanim->Update();
		...
		case 3307:  // “过会儿，我还会添加这个！”
		{
			...
			AttachEffect* anAttachEffect = AttachReanim(aCrazyDaveReanim->GetTrackInstanceByName("Dave_pot")->mAttachmentID, aReanimMagnet, 0, 0);
			TodScaleRotateTransformMatrix(anAttachEffect->mOffset, 49 - 25, 25 + 25, 0, 1.2f, 1.2f);
			aCrazyDaveReanim->Update();
			break;
		}
