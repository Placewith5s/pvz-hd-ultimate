...
Zombie* Projectile::FindCollisionTarget()
{
	...
	while (mBoard->IterateZombies(aZombie))
	{
		...
		if (mProjectileType == ProjectileType::PROJECTILE_ZOMBIE_CABBAGE &&
			aZombie->mMindControlled) {
			isEffected = aZombie->EffectedByDamage(258);
		}
...
void Projectile::CheckForCollision()
{
	...
	if (mProjectileType == ProjectileType::PROJECTILE_ZOMBIE_CABBAGE)
	{
		...
		if (aPlant)
		{
			...
			mApp->PlayFoley(FoleyType::FOLEY_SPLAT);
			TodParticleSystem* aParticle = mApp->AddTodParticle(mPosX - 3.0f, mPosY + 17.0f, mRenderOrder + 1, ParticleEffect::PARTICLE_CABBAGE_SPLAT);
			if (aParticle)
			{
				aParticle->OverrideColor(nullptr, mOverrideColor);
				if (mEnableExtraAdditiveDraw)
