	...
	if (IsAdventureMode())
	{
		if (abs(mBoard->mLevel) == FINAL_LEVEL)
		{
			if (mPlayerInfo->mFinishedAdventure == 1)
			{
				if (mBoard->mLevel >= 0)
				{
					mPlayerInfo->SetLevel(61);
					mPlayerInfo->mFinishedAdventure++;
				}
				else
				{
					mPlayerInfo->SetLevel(-59);
				}
			}
			else
			{
...
void LawnApp::CheckForGameEnd()
{
	...
	if (IsAdventureMode())
	{
		...
		if (!isReplaying)
		{
			if (IsFirstTimeAdventureMode() && aLevel < 60)
			{
			...
			else if (aLevel == 9 || aLevel == 19 || aLevel == 29 || aLevel == 39 ||
				aLevel == 49 || aLevel == 59)
			{
				ShowAwardScreen(AwardType::AWARD_FORLEVEL, true);
			}
...
bool LawnApp::IsNightRoofMiniGameLevel()
{
	if (mBoard == nullptr)
		return false;
...
bool LawnApp::IsGreenHouseMiniGameLevel()
{
	if (mBoard == nullptr)
		return false;

	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_WORLD_7_5)
		return true;
...
bool LawnApp::IsMiniBossLevel()
{
	...
	return
		...
		(IsAdventureMode() && mPlayerInfo->mLevel == 50);
}
...
bool LawnApp::IsNight()
{
	...
	if (mBoard)
		return (mBoard->mLevel >= 11 && mBoard->mLevel <= 20) || (mBoard->mLevel >= 31 && mBoard->mLevel <= 40) ||
		(mBoard->mLevel >= 51 || mBoard->mLevel == 60);

	return (mPlayerInfo->mLevel >= 11 && mPlayerInfo->mLevel <= 20) || (mPlayerInfo->mLevel >= 31 && mPlayerInfo->mLevel <= 40) ||
		(mPlayerInfo->mLevel >= 51 || mPlayerInfo->mLevel == 60);
}
...
int LawnApp::TrophiesNeedForGoldSunflower()
{
	...
	return 85 - GetNumTrophies(CHALLENGE_PAGE_SURVIVAL) - GetNumTrophies(CHALLENGE_PAGE_CHALLENGE) - GetNumTrophies(CHALLENGE_PAGE_CUSTOM_CHALLENGE) - GetNumTrophies(CHALLENGE_PAGE_CUSTOM_CHALLENGE_2) - GetNumTrophies(CHALLENGE_PAGE_PUZZLE) - GetNumTrophies(CHALLENGE_PAGE_LAST_STAND);
}
